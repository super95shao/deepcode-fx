...
|---------|----------|-----------|
| 差异汇总 / Diff Summary | 无 | 可折叠的 `Changes` 汇总，显示 `+N -M (N files)`，每个文件有独立的增删统计和可点击行号 |
| 差异编辑匹配 / Edit Matching | 基础精确匹配 | 逐行标准化对比：容忍 tab/空格缩进差异、行尾空白、Read 工具行号前缀 |
| 行号跳转 / Line-number Jump | 无 | 所有 diff 预览中的行号皆可点击，直接打开 VSCode 对应文件并跳转到该行 |
...
# 4. Share the generated .vsix file / 分享生成的 .vsix 文件
```
