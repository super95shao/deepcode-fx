...
export async function handleEditTool(
  ...
  return executeValidatedTool(
    ...
    async (input) => {
      ...
      let snippet = snippetId ? getSnippet(context.sessionId, snippetId) : null;
      ...
      try {
        ...
        let scope = buildSearchScope(filePath, raw, lineIndex, snippet ?? null);
        let matches = findOccurrences(raw, oldString, scope);
        let matchedVia: "exact" | "line_leading_tab_correction" | "normalized" | "loose_escape" | "llm_escape_correction" = "exact";
        ...
        if (matches.length === 0) {
          ...
          // (handles Read line-number prefixes, tab/space diffs, trailing whitespace)
          const normalizedMatches = findNormalizedOccurrences(raw, oldString, scope, lineIndex);
          if (normalizedMatches) {
            // Verify uniqueness: if we get multiple matches via normalized matching,
            // fall through to let the normal matching+error flow handle it
            const strippedOldString = stripReadLineNumberPrefixes(oldString);
            if (normalizedMatches.length === 1) {
              matches = normalizedMatches;
              matchedVia = "normalized";
              replacementOldString = strippedOldString;
              replacementNewString = stripReadLineNumberPrefixes(newString);
            } else {
              // For multiple normalized matches, do a secondary check with the
              // original oldString to see if this is really just one exact match
              const exactMatches = findOccurrences(raw, strippedOldString, scope);
        ...
        if (matches.length === 0) {
          // snippet 范围没命中，退回到全文件搜索
          if (snippet && scope) {
            const fullScope: SearchScope = { filePath, startOffset: 0, endOffset: raw.length, startLine: 1, endLine: lineIndex.lines.length, snippetId: null };
            const fullMatches = findOccurrences(raw, oldString, fullScope);
...
/**
 * Strips Read tool line-number prefixes from old_string.
 * Read output format: "     1\tcontent" → "content"
 * Only strips when prefix looks like a Read-style line number
 * (≥2 leading spaces before digits+tab), to avoid false positives
 * on actual code content.
 */
function stripReadLineNumberPrefixes(value: string): string {
  return value
    .split("\n")
    .map((line) => line.replace(/^ {2,}\d+\t/, ""))
    .join("\n");
}

/**
 * Normalizes a line of text for comparison purposes only.
 * This does NOT affect the replacement — it only helps find match positions.
 */
function normalizeLineForMatch(line: string): string {
  return line
    .replace(/^ {2,}\d+\t/, "") // strip Read line number prefix
    .replace(/\s+$/, "")        // strip trailing whitespace
    .replace(/\t/g, " ");       // normalize tabs to spaces for comparison
}

/**
 * Finds occurrences using line-by-line normalized matching.
 * Tolerates:
 * - Read line number prefixes in old_string
 * - Tab vs space indentation differences
 * - Trailing whitespace differences
 *
 * Returns match positions in the original (non-normalized) raw content,
 * so the replacement always uses the exact original content.
 */
function findNormalizedOccurrences(
  raw: string,
  oldString: string,
  scope: SearchScope,
  lineIndex: LineIndex
  ...
  const scopeText = raw.slice(scope.startOffset, scope.endOffset);
  const rawLines = scopeText.split("\n");
  const needleLines = oldString.split("\n");

  if (needleLines.length === 0 || rawLines.length < needleLines.length) {
    return null;
  }

  // Normalize all lines for comparison
  const normalizedNeedle = needleLines.map(normalizeLineForMatch);
  const normalizedRaw = rawLines.map(normalizeLineForMatch);

  // Sliding window: find sequences of matching lines
  const matches: MatchOccurrence[] = [];
  for (let start = 0; start <= normalizedRaw.length - normalizedNeedle.length; ) {
    let allMatch = true;
    for (let i = 0; i < normalizedNeedle.length; i++) {
      if (normalizedNeedle[i] !== normalizedRaw[start + i]) {
        allMatch = false;
        break;
      }
    }
    ...
    if (allMatch) {
      // Map back to raw content offsets using lineIndex
      const fileStartLine = scope.startLine + start;
      const fileEndLine = scope.startLine + start + normalizedNeedle.length - 1;
      matches.push({
        startOffset: lineIndex.lineStarts[fileStartLine],
        endOffset: lineIndex.lineStarts[fileEndLine + 1],
